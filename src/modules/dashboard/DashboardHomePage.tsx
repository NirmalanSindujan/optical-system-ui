...
import {
  ...
  type BusinessSummaryBranchCash,
...
const BANK_LEDGER_PAGE_SIZE = 20;
...
function getDirectionToneClass(
  direction: CashLedgerEntry["direction"] | BankLedgerEntry["direction"],
) {
...
function getEntryTypeBadgeProps(entryType: string) {
  switch (entryType) {
...
function formatEntryTypeLabel(entryType: string) {
  switch (entryType) {
    ...
    case "CUSTOMER_CHEQUE_CLEARANCE":
      return "Customer Cheque Clearance";
    default:
...
function getTransactionStatusBadgeClass(status: string) {
  switch (status) {
    case "POSTED":
...
function DashboardHomePage() {
  ...
  const [selectedCashBranchId, setSelectedCashBranchId] = useState<
    number | null
  >(null);
  const [cashLedgerFromDate, setCashLedgerFromDate] = useState(
    getMonthStartDateValue(),
  );
  const [cashLedgerToDate, setCashLedgerToDate] = useState(getTodayDateValue());
  const [cashLedgerPage, setCashLedgerPage] = useState(0);
  const [bankLedgerFromDate, setBankLedgerFromDate] = useState(
    getMonthStartDateValue(),
  );
  const [bankLedgerToDate, setBankLedgerToDate] = useState(getTodayDateValue());
  const [bankLedgerPage, setBankLedgerPage] = useState(0);
  ...
  const selectedBranch = useMemo(
    () =>
      branchOptions.find(
        (branch) => branch.branchId === selectedCashBranchId,
      ) ?? null,
    [branchOptions, selectedCashBranchId],
  );
  ...
  const cashLedgerQuery = useQuery({
    ...
    enabled:
      cashLedgerOpen && canViewBusinessSummary && Boolean(selectedCashBranchId),
  });
  ...
  const bankLedgerQuery = useQuery({
    queryKey: [
      "dashboard",
      "bank-ledger",
      bankLedgerFromDate,
    ...
    queryFn: () =>
      getBankLedger({
        fromDate: bankLedgerFromDate || undefined,
        toDate: bankLedgerToDate || undefined,
        page: bankLedgerPage,
        size: BANK_LEDGER_PAGE_SIZE,
      }),
    enabled: bankLedgerOpen && canViewBusinessSummary,
  });

  useEffect(() => {
    if (!bankLedgerQuery.isError) return;
    toast({
      variant: "destructive",
      title: "Failed to load bank ledger",
      description: getApiErrorMessage(bankLedgerQuery.error),
    });
  }, [bankLedgerQuery.error, bankLedgerQuery.isError, toast]);
  ...
  const openBankLedger = () => {
    setBankLedgerPage(0);
    setBankLedgerOpen(true);
  };
  ...
  return (
    <div className="space-y-4">
            ...
            <>
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                {Array.from({ length: 4 }).map((_, index) => (
                  <Card
                    key={index}
                    className="min-h-[7.5rem] animate-pulse border-border/70 bg-muted/40"
                  />
                ))}
              </div>
            ...
            <>
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                ...
                <SummaryMetricCard
                  ...
                  icon={Landmark}
                  onClick={openBankLedger}
                  actionHint="Click to view bank ledger."
                />
                <SummaryMetricCard
              ...
              <Card className="border-border/70 shadow-sm">
                <CardHeader className="pb-3">
                  <CardTitle className="flex items-center justify-between  gap-2 text-base">
                   <p className="flex items-center   gap-2 text-base"> <Building2 className="h-4 w-4 text-primary" />
                    Branch Cash In Hand
                    ...
                    >
                      {businessSummaryQuery.isFetching
                        ? "Refreshing..."
                        : "Refresh"}
                    </Button>
                  </CardTitle>
                  <CardDescription>
                    Includes direct cash receipts and received cheques cleared
                    to cash.
                  </CardDescription>
                </CardHeader>
                <CardContent>
      ...
      <Sheet open={cashLedgerOpen} onOpenChange={setCashLedgerOpen}>
        <SheetContent
          side="right"
          className="w-full overflow-y-auto border-l border-border/70 p-0 sm:max-w-6xl"
        >
          <SheetHeader className="border-b px-6 py-5">
            <SheetTitle>Cash Ledger</SheetTitle>
            <SheetDescription>
              Branch-level cash income and outgoing ledger for the selected
              period.
            </SheetDescription>
          </SheetHeader>
          ...
          <div className="space-y-4 px-6 py-5">
              ...
              <Card className="border-border/70 shadow-sm">
                <CardContent className="p-4">
                  ...
                  <p className="mt-1 text-lg font-semibold">
                    {cashLedgerQuery.data?.branchCode ||
                      selectedBranch?.branchCode ||
                      "-"}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {cashLedgerQuery.data?.branchName ||
            ...
            <Card className="border-border/70 shadow-sm">
              <CardHeader className="pb-3">
                ...
                <CardDescription>
                  {cashLedgerQuery.data?.fromDate || cashLedgerFromDate || "-"}{" "}
                  to {cashLedgerQuery.data?.toDate || cashLedgerToDate || "-"}
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="overflow-x-auto">
                  <Table className="min-w-[1120px] table-fixed">
                    ...
                    <TableBody>
                      ...
                      cashLedgerQuery.isFetching ? (
                        ...
                        <TableRow>
                          <TableCell colSpan={8}>
                            Select a branch to view the cash ledger.
                          </TableCell>
                        </TableRow>
                      ) : (cashLedgerQuery.data?.entries?.length ?? 0) === 0 ? (
                        <TableRow>
                          <TableCell colSpan={8}>
                            No cash ledger entries found for this period.
                          </TableCell>
                        </TableRow>
                      ) : (
                        cashLedgerQuery.data?.entries.map((entry) => {
                          const typeBadge = getEntryTypeBadgeProps(
                            entry.entryType,
                          );

                          return (
                            <TableRow
                              key={`${entry.entryType}-${entry.transactionId}-${entry.createdAt}`}
                            >
                              <TableCell>
                ...
                <div className="mt-4 flex items-center justify-between gap-3 border-t pt-4">
                  <p className="text-sm text-muted-foreground">
                    Page {(cashLedgerQuery.data?.page ?? cashLedgerPage) + 1} of{" "}
                    {Math.max(1, cashLedgerQuery.data?.totalPages ?? 1)} (
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      onClick={() =>
                        setCashLedgerPage((current) => current - 1)
                      }
                    >
                      Previous
                    </Button>
      ...
      <Sheet open={bankLedgerOpen} onOpenChange={setBankLedgerOpen}>
        <SheetContent
          side="right"
          className="w-full overflow-y-auto border-l border-border/70 p-0 sm:max-w-6xl"
        >
          <SheetHeader className="border-b px-6 py-5">
            <SheetTitle>Bank Ledger</SheetTitle>
            <SheetDescription>
              Bank income and outgoing ledger for the selected period.
            </SheetDescription>
          </SheetHeader>

          <div className="space-y-4 px-6 py-5">
            <div className="grid gap-4 md:grid-cols-[minmax(170px,1fr)_minmax(170px,1fr)_auto]">
              <Input
                type="date"
                value={bankLedgerFromDate}
                onChange={(event) => {
                  setBankLedgerFromDate(event.target.value);
                  setBankLedgerPage(0);
                }}
              />
              <Input
                type="date"
                value={bankLedgerToDate}
                onChange={(event) => {
                  setBankLedgerToDate(event.target.value);
                  setBankLedgerPage(0);
                }}
              />
              <Button
                variant="outline"
                onClick={() => bankLedgerQuery.refetch()}
                disabled={bankLedgerQuery.isFetching}
              >
                <RefreshCcw className="mr-2 h-4 w-4" />
                {bankLedgerQuery.isFetching ? "Refreshing..." : "Refresh"}
              </Button>
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
              <Card className="border-border/70 shadow-sm">
                <CardContent className="p-4">
                  ...
                  <p className="mt-1 text-lg font-semibold">
                    {bankLedgerQuery.data?.bankCode || "-"}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {bankLedgerQuery.data?.bankName || "Primary bank account"}
                  </p>
                </CardContent>
              </Card>
              <Card className="border-border/70 shadow-sm">
                <CardContent className="p-4">
                  ...
                  <p className="mt-1 text-lg font-semibold text-emerald-700">
                    {formatMoney(bankLedgerQuery.data?.totalIncome ?? 0)}
                  </p>
                </CardContent>
              </Card>
              <Card className="border-border/70 shadow-sm">
                <CardContent className="p-4">
                  ...
                  <p className="mt-1 text-lg font-semibold text-rose-700">
                    {formatMoney(bankLedgerQuery.data?.totalOutgoing ?? 0)}
                  </p>
                </CardContent>
              </Card>
              <Card className="border-border/70 shadow-sm">
                <CardContent className="p-4">
                  ...
                  <p className="mt-1 text-lg font-semibold">
                    {formatMoney(bankLedgerQuery.data?.netMovement ?? 0)}
                  </p>
                </CardContent>
              </Card>
              <Card className="border-border/70 shadow-sm">
                <CardContent className="p-4">
                  ...
                  <p className="mt-1 text-lg font-semibold">
                    {bankLedgerQuery.data?.totalCounts ?? 0}
                  </p>
                </CardContent>
              </Card>
            </div>
            ...
            <Card className="border-border/70 shadow-sm">
              <CardHeader className="pb-3">
                ...
                <CardDescription>
                  {bankLedgerQuery.data?.fromDate || bankLedgerFromDate || "-"} to{" "}
                  {bankLedgerQuery.data?.toDate || bankLedgerToDate || "-"}
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="overflow-x-auto">
                  <Table className="min-w-[1280px] table-fixed">
                    ...
                    <TableBody>
                      {bankLedgerQuery.isLoading || bankLedgerQuery.isFetching ? (
                        <TableRow>
                          <TableCell colSpan={9}>Loading bank ledger...</TableCell>
                        </TableRow>
                      ) : (bankLedgerQuery.data?.entries?.length ?? 0) === 0 ? (
                        <TableRow>
                          <TableCell colSpan={9}>
                            No bank ledger entries found for this period.
                          </TableCell>
                        </TableRow>
                      ) : (
                        bankLedgerQuery.data?.entries.map((entry) => {
                          const typeBadge = getEntryTypeBadgeProps(
                            entry.entryType,
                          );
                          ...
                          return (
                            ...
                            >
                              ...
                              <TableCell>
                                ...
                                >
                                  {formatEntryTypeLabel(entry.transactionStatus)}
                                </Badge>
                              </TableCell>
                              <TableCell>
                                <span
                ...
                <div className="mt-4 flex items-center justify-between gap-3 border-t pt-4">
                  <p className="text-sm text-muted-foreground">
                    Page {(bankLedgerQuery.data?.page ?? bankLedgerPage) + 1} of{" "}
                    {Math.max(1, bankLedgerQuery.data?.totalPages ?? 1)} (
                    {bankLedgerQuery.data?.totalCounts ?? 0} total)
                  </p>
                  <div className="flex gap-2">
                    <Button
                      variant="outline"
                      disabled={
                        bankLedgerPage <= 0 ||
                        bankLedgerQuery.isLoading ||
                        bankLedgerQuery.isFetching
                      }
                      onClick={() =>
                        setBankLedgerPage((current) => current - 1)
                      }
                    >
                      Previous
                    </Button>
