...
} from "@/modules/products/singleVision.validation";
...
type PowerFieldName = "sph" | "cyl" | "sphStart" | "sphEnd" | "cylStart" | "cylEnd";
...
function SingleVisionCreateDrawer({ open, onClose, onSaved }: SingleVisionCreateDrawerProps) {
  ...
  const saveMutation = useMutation({
    mutationFn: (values: SingleVisionFormValues) => createSingleVision(buildSingleVisionPayload(values)),
    onSuccess: async (response) => {
      await queryClient.invalidateQueries({ queryKey: ["products"] });
      await queryClient.invalidateQueries({ queryKey: ["products", "lenses", "subtabs"] });
      toast({
  ...
  const renderPowerSelect = (fieldName: PowerFieldName, label: string, options: string[]) => (
    <div>
      ...
      <Controller
        ...
        render={({ field, fieldState }) => (
          <>
            ...
            />
            {fieldState.error?.message ? <p className="mt-1 text-xs text-destructive">{fieldState.error.message}</p> : null}
          </>
        )}
      />
    </div>
      ...
      <SheetContent side="right" hideClose className="max-w-2xl overflow-y-auto p-6 sm:max-w-2xl">
        ...
        <form className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit((values) => saveMutation.mutate(values))}>
          <div>
            ...
            <Input id="companyName" autoFocus placeholder="ABC Optical" {...register("companyName")} />
          ...
          <div>
            ...
            {errors.material ? <p className="mt-1 text-xs text-destructive">{errors.material.message}</p> : null}
          </div>
          ...
          <div>
            ...
            <Input id="index" type="number" step="0.01" placeholder="1.50" {...register("index")} />
            {errors.index ? <p className="mt-1 text-xs text-destructive">{errors.index.message}</p> : null}
          </div>
          ...
          <div>
            ...
            <Input id="extra" placeholder="Pair" {...register("extra")} />
          </div>
                ...
                <div className="flex flex-wrap gap-4">
                  {SINGLE_VISION_ADDITION_METHOD_VALUES.map((method) => (
                    <label key={method} className="inline-flex items-center gap-2 text-sm">
                      <input type="radio" value={method} {...register("additionMethod")} />
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              {additionMethod === "SINGLE" ? (
                ...
                <>
                  ...
                  {cylEnabled ? renderPowerSelect("cylStart", "CYL Start", CYL_OPTIONS) : null}
                  {cylEnabled ? renderPowerSelect("cylEnd", "CYL End", CYL_OPTIONS) : null}
                </>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <Input id="purchasePrice" type="number" min={0} step="0.01" placeholder="1200.00" {...register("purchasePrice")} />
          ...
          <div>
            ...
            <Input id="sellingPrice" type="number" min={0} step="0.01" placeholder="1800.00" {...register("sellingPrice")} />
            {errors.sellingPrice ? (
          ...
          <div className="md:col-span-2 flex justify-end gap-2">
            ...
            <Button type="submit" disabled={isSubmitting || saveMutation.isPending}>
              {isSubmitting || saveMutation.isPending ? "Saving..." : "Create Single Vision"}
            </Button>
          </div>
        </form>
      </SheetContent>
