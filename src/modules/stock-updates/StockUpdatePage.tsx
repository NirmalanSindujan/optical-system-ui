...
function StockPurchasePreviewCard({
  ...
  return (
    <div className="flex h-full min-h-[24rem] flex-col rounded-3xl border bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(248,250,252,0.98))] shadow-sm">
      ...
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4">
        ...
        <div className="space-y-3">
          ...
          {record.items.map((line: StockPurchaseItemResponse) => (
            <div key={line.id} className="rounded-2xl border bg-background/80 p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="font-medium">{line.productName}</p>
                  <p className="text-sm text-muted-foreground">{line.sku}</p>
                </div>
                <Badge variant="outline">{formatMoney(line.lineTotal)} {record.currencyCode}</Badge>
              </div>
              <div className="mt-3 grid gap-3 sm:grid-cols-3">
                <div>
                  <p className="text-xs uppercase tracking-[0.14em] text-muted-foreground">Variant ID</p>
                  <p className="mt-1 text-sm font-medium">{line.variantId}</p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.14em] text-muted-foreground">Quantity</p>
                  <p className="mt-1 text-sm font-medium">{formatMoney(line.quantity)}</p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.14em] text-muted-foreground">Purchase Price</p>
...
function StockUpdatePage() {
  ...
  const defaultTab = location.pathname.endsWith("/add") ? "add" : "list";
  ...
  const onPreviewSave = () => {
    toast({
      ...
      description: "Share the stock update endpoint and this tab can be connected to the backend."
    });
  };
  ...
  const onDraftLineChange = (
    ...
    setDraftItems((current) =>
      current.map((item) => {
        ...
        const nextValue =
          field === "previousQty" || field === "changeQty" ? Number(value || 0) : value;
  ...
  return (
    <Card className="flex min-h-[calc(100svh-11rem)] flex-col overflow-hidden">
      <CardHeader className="border-b pb-4">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div className="space-y-1">
            ...
            <CardDescription>
              Create and review stock corrections, returns, transfers, and damage adjustments.
            </CardDescription>
          </div>
          <div className="flex flex-wrap gap-2">
            <Badge variant="outline" className="rounded-full px-3 py-1">
              Customer-style list pattern
            </Badge>
            <Badge variant="outline" className="rounded-full px-3 py-1">
              Endpoint-ready sample UI
            </Badge>
          </div>
        </div>
      </CardHeader>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex h-full min-h-0 flex-col gap-4">
          <TabsList className="grid w-full grid-cols-2 sm:w-[420px]">
            ...
            <TabsTrigger value="list" className="gap-2">
              ...
              Display All Stockupdate List
            </TabsTrigger>
          </TabsList>

          <TabsContent value="add" className="min-h-0 flex-1">
            <div className="grid h-full gap-4 xl:grid-cols-[minmax(0,1.2fr)_440px]">
              <section className="min-h-0 overflow-y-auto rounded-3xl border bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(248,250,252,0.98))] shadow-sm">
                <div className="border-b px-5 py-4">
                  <h3 className="text-lg font-semibold">Sample Add Stock Update Form</h3>
                  <p className="text-sm text-muted-foreground">
                    Layout only for now. Once you share the endpoint, this can be submitted like the customer flow.
                  </p>
                </div>
                        ...
                        <div key={line.id} className="rounded-2xl border bg-background/80 p-4">
                          <div className="grid gap-3 lg:grid-cols-[minmax(0,1.4fr)_140px_120px_1fr_auto]">
                            <Input
                              ...
                              onChange={(event) =>
                                onDraftLineChange(line.id, "productName", event.target.value)
                              }
                            ...
                            <Input
                              ...
                              onChange={(event) =>
                                onDraftLineChange(line.id, "previousQty", event.target.value)
                              }
                              placeholder="Before"
                            />
                            <Input
                              ...
                              onChange={(event) =>
                                onDraftLineChange(line.id, "changeQty", event.target.value)
                              }
                              placeholder="Change"
                            />
          ...
          <TabsContent value="list" className="min-h-0 flex-1">
            <div className="grid h-full gap-4 xl:grid-cols-[minmax(0,1.35fr)_420px]">
              <section className="flex min-h-0 flex-col rounded-3xl border bg-card/70 shadow-sm">
                <div className="border-b px-5 py-4">
